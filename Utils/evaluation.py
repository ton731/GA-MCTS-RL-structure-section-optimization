...
def designScore(designer, graph, response, design):
    '''
    Score will be contributed by PlasticHingeScore and MaterialUsageScore
    '''
    strengthFactor, materialFactor = 0.7, 0.3
    score = 0
    # score += strengthFactor * plasticHingeScore(designer, graph, response)
    score += materialFactor * materialUsageScore(designer, design)
    return score



def driftRatioScore(designer, graph, response):
    pass



def materialUsageScore(designer, candidate_design):
    min_usage, max_usage = designer.min_max_usage

    # get design's total material usage
    total_usage = 0
    for i, section in enumerate(candidate_design):
        total_usage += designer.element_length_list[i] * designer.area_dict[section]
...
My_start_index = 15
section_info_dim = 2
def plasticHingeScore(designer, graph, response):
    ...
    for node_index in range(ph_pred.shape[0]):
        for i, face_index in enumerate(list(range(3, 9))):   # face_index is index for Mz(x_n, x_p, y_n, y_p, z_n, z_p)
            ...
            if My_localZ_face_i <= 0.1:     # It means this face is not connect to any element
                continue
            pred_node_plastic_hinge = (abs(pred_moment_face_i) >= designer.yield_factor * My_localZ_face_i) + 0  # [2000]
